...
class TestDateOffset:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"microseconds": 1, "milliseconds": 1}, "2028-01-01 00:00:00.001001"),
            ({"seconds": 1, "milliseconds": 1}, "2028-01-01 00:00:01.001"),
            ({"minutes": 1, "milliseconds": 1}, "2028-01-01 00:01:00.001"),
            ({"hours": 1, "milliseconds": 1}, "2028-01-01 01:00:00.001"),
            ({"days": 1, "milliseconds": 1}, "2028-01-02 00:00:00.001"),
            ({"weeks": 1, "milliseconds": 1}, "2028-01-08 00:00:00.001"),
            ({"months": 1, "milliseconds": 1}, "2028-02-01 00:00:00.001"),
            ({"years": 1, "milliseconds": 1}, "2025-01-01 00:00:00.001"),
        ],
    )
    def test_milliseconds_combination(self, offset_kwargs, expected_arg):
        # GH 49897
        offset = DateOffset(**offset_kwargs)
        ts = Timestamp("2028-01-01")
        result = ts + offset
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            Timestamp("2028-01-01 00:07:00.000000018"),
        ),
        (DateOffset(nanoseconds=3), Timestamp("2028-01-01 00:00:00.000000003")),
    ],
)
def test_dateoffset_add_sub_timestamp_series_with_nano(offset, expected):
    # GH 47856
    start_time = Timestamp("2028-01-01")
...
def test_dateoffset_operations_on_dataframes():
    ...
    expecteddate = Timestamp("2027-06-30")
