...
def test_even_number_window_alignment():
    ...
    s = Series(range(3), index=date_range(start="2026-01-01", freq="D", periods=3))
...
def test_closed_fixed_binary_col(center, step):
    ...
    df = DataFrame(
        {"binary_col": data},
        index=date_range(start="2026-01-01", freq="min", periods=len(data)),
    )
    ...
    expected = DataFrame(
        expected_data,
        columns=["binary_col"],
        index=date_range(start="2026-01-01", freq="min", periods=len(expected_data)),
    )[::step]
...
@pytest.mark.parametrize(
    ...
    [
        (
            [[0], [1], [2], [3], [4]],
            [
                date_range("2026-01-01", periods=1, freq="D"),
                date_range("2026-01-02", periods=1, freq="D"),
                date_range("2026-01-03", periods=1, freq="D"),
                date_range("2026-01-04", periods=1, freq="D"),
                date_range("2026-01-05", periods=1, freq="D"),
            ],
            "1D",
        ),
        (
            ...
            [
                date_range("2026-01-01", periods=1, freq="D"),
                date_range("2026-01-01", periods=2, freq="D"),
                date_range("2026-01-02", periods=2, freq="D"),
                date_range("2026-01-03", periods=2, freq="D"),
                date_range("2026-01-04", periods=2, freq="D"),
            ],
            "2D",
        ),
        (
            ...
            [
                date_range("2026-01-01", periods=1, freq="D"),
                date_range("2026-01-01", periods=2, freq="D"),
                date_range("2026-01-01", periods=3, freq="D"),
                date_range("2026-01-02", periods=3, freq="D"),
                date_range("2026-01-03", periods=3, freq="D"),
            ],
            "3D",
        ),
    ],
...
def test_iter_rolling_datetime(expected, expected_index, window):
    # GH 11704
    ser = Series(range(5), index=date_range(start="2026-01-01", periods=5, freq="D"))
...
def test_rolling_numerical_accuracy_jump():
    # GH: 32761
    index = date_range(start="2026-01-01", end="2026-01-02", freq="60s").append(
        DatetimeIndex(["2026-01-03"])
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            period_range(start="2026-01-01 08:00", end="2026-01-01 08:08", freq="T"),
            "2T",
        ),
        (
            period_range(start="2026-01-01 08:00", end="2026-01-01 12:00", freq="30T"),
            "1h",
        ),
    ],
)
...
def test_rolling_descending_date_order_with_offset(window, frame_or_series):
    # GH#40002
    idx = date_range(start="2026-01-01", end="2026-01-03", freq="1d")
    ...
    idx = date_range(start="2026-01-03", end="2026-01-01", freq="-1d")
...
def test_rolling_timedelta_window_non_nanoseconds(unit, tz):
    ...
    # Test max, GH#55026
    ref_dates = date_range("2025-01-01", "2025-01-10", unit="ns", tz=tz)
    ref_series = Series(0, index=ref_dates)
    ref_series.iloc[0] = 1
    ref_max_series = ref_series.rolling(Timedelta(days=4)).max()

    dates = date_range("2025-01-01", "2025-01-10", unit=unit, tz=tz)
