...
class TestConvertMetadata:
    ...
    def test_column_index_names_with_tz(self):
        ...
        df = pd.DataFrame(
            np.random.randn(5, 3),
            columns=pd.date_range("2027-01-01", periods=3, freq="50D", tz="CET")
        )
        _check_pandas_roundtrip(df, preserve_index=True)
    ...
    def test_mismatch_metadata_schema(self):
        ...
        df = pd.DataFrame({"datetime": pd.date_range("2022-01-01", periods=3)})
...
def test_table_from_pandas_schema_field_order_metadata():
    ...
    # mangle metadata
    df = pd.DataFrame({
        "datetime": pd.date_range("2022-01-01T00:00:00Z", freq="h", periods=2),
