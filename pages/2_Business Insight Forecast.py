# pages/2_Business Insight Forecast.py
import os
import sys
from pathlib import Path
import json
from models.forecasting_advanced import calculate_forecast_accuracy
import pandas as pd
...
# your utilities & models (assumes these modules exist)
from utils.kpi_calculator import calculate_daily_sales, compute_kpis
...
def load_data_from_db():
    """Load merged sales + product info from DB, compute TotalSales and Date dtype."""
    try:
        ...
        query = (
            ...
            "FROM sales s JOIN product_info p USING(`Item Code`)"
        )
        ...
        if 'TotalSales' not in df.columns:
            df['TotalSales'] = df.get('Quantity Sold (kilo)', 0) * df.get('Unit Selling Price (RMB/kg)', 0)
...
if 'df' not in st.session_state or st.session_state['df'] is None:
    ...
    if df is None or df.empty:
        st.warning("⚠️ No data available. Please load sales data from the Main Page first.")
        st.stop()
    ...
    try:
        ...
        tmp = df.copy()
        if 'TotalSales' not in tmp.columns:
            tmp['TotalSales'] = tmp.get('Quantity Sold (kilo)', 0) * tmp.get('Unit Selling Price (RMB/kg)', 0)
        tmp['Date'] = pd.to_datetime(tmp['Date'], errors='coerce')
        daily = tmp.groupby(tmp['Date'].dt.date)['TotalSales'].sum().rename("daily_sales").reset_index()
